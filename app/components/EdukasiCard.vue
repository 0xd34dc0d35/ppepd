...
<template>
  <NuxtLink :to="`/edukasi/${item.id}`" class="group block bg-white rounded-md border border-brand-green/5 hover:border-brand-green/25 hover:shadow-xl hover:shadow-brand-green/8 transition-all duration-500 overflow-hidden flex flex-col h-full">
    <!-- Image -->
    <div class="relative aspect-[16/8] overflow-hidden bg-brand-charcoal/10">
      <img
        :src="item.image"
        :alt="item.title"
        ...
        loading="lazy"
      />
      <div class="absolute inset-0 bg-gradient-to-t from-brand-charcoal/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-4">
      ...
      <div class="absolute top-4 left-4 flex gap-2">
        <span :class="[
          ...
          item.type === 'edukasi'
            ? 'bg-brand-green/90 backdrop-blur-md text-white'
    ...
    <div class="p-4 flex flex-col flex-grow">
      <div class="flex justify-between items-start mb-2">
        ...
        <span :class="[
          ...
          item.type === 'edukasi' ? 'text-brand-green/60' : 'text-brand-orange/60'
        ]">{{ item.type }}</span>
      </div>
      ...
      <div class="flex items-center justify-between mt-auto pt-3 border-t border-brand-green/5">
        <p class="text-[11px] text-brand-charcoal/50 font-medium line-clamp-1">
          <span class="text-brand-charcoal/70 font-semibold">{{ item.author }}</span>
        </p>
